...
import DailyTable from './tableDaily';
...
export default class Default extends Component {
    ...
    onSubmit(data, filter = false){
        ...
        axios.get(API().getDaily + '?' + u, header(localStorage.getItem('token')))
        .then((response) => {
            let res = response.data;
            let result = SwalResult(res);
            if(result){
    ...
    refreshReport(){
        ...
        axios.post(API().postProsesDaily, params(this.state.params), header(localStorage.getItem('token')))
        .then((response) => {
            let res = response.data;
            let result = SwalResult(res);
    ...
    downloadReport(tipe=1){
        ...
        let u = new URLSearchParams(this.state.params).toString();
        axios.get(API().getExcelDaily + '?' + u, header(localStorage.getItem('token')))
        .then((response) => {
            Swal.close();
            this.setState({isProses : false})
