...
export default class Default extends Component {
    ...
    onSubmit(data, filter = false){
        ...
        axios.get(API().getMaintenance + "?status=1&" + this.state.query + '&' + u, header(localStorage.getItem('token')))
        .then((response) => {
            let res = response.data;
            let result = SwalResult(res);
            if(result){
    ...
    downloadReport(tipe=1){
        let u = new URLSearchParams(this.state.params).toString();
        const param = '?status=1&' +u;
        if(tipe==1){
            axios.get(API().exportReport + param, header(localStorage.getItem('token')))
            .then((response) => {
            ...
            axios.get(API().exportLampiran + param, header(localStorage.getItem('token')))
            .then((response) => {
                window.open(response.data.path);
            }).catch((error) => {
                Swal.fire({
                            ...
                            <Card>
                                <CardHeader>
                                    ...
                                    <Dropdown isOpen={this.state.dropDown} toggle={() => {
                                        ...
                                        <DropdownMenu>
                                            <DropdownItem onClick={()=> this.downloadReport(1)}><i style={{color:'#000000'}} className="fa fa-file-pdf-o"></i>1.A. Report</DropdownItem>
                                            <DropdownItem onClick={()=> this.downloadReport(2)}><i style={{color:'#000000'}} className="fa fa-file-pdf-o"></i>1.B. Lampiran</DropdownItem>
                                        </DropdownMenu>
                                    </Dropdown>
                                    </div>
                                </CardHeader>
