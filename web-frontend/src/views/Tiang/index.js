...
function Add(props){
  ...
  const onSubmit = data => {
    Swal.fire({
      ...
      if(result.value){
        setLoading(true);
        axios.post(API().postMasterTiang, params(data), header(localStorage.getItem('token')))
        .then((response) => {
...
function Update(props){
  const { register, handleSubmit, errors, reset } = useForm({
    defaultValues: {
      tinggi: props.data.ketinggian,
      tipe  : props.data.nama_tipe
    }
  });
  const [loading, setLoading] = useState(false);
  ...
  const onSubmit = data => {
    Swal.fire({
      ...
      if(result.value){
        setLoading(true);
        axios.post(API().updateMasterTiang, params(data), header(localStorage.getItem('token')))
        .then((response) => { 
            let res = response.data;
            let result = SwalResult(res);
      ...
      <Row>
          <Col xs="12" sm="12" md="12">
              <Card>
                  ...
                  <CardBody>
                    <Form onSubmit={handleSubmit(onSubmit)}>
                      <input ref={register()} type="hidden" name="id" value={props.data.id_tipe_tiang}/>
                      <InputForm 
                        refs={register({
                            required : true,
                            maxLength: {value: 100, message: "100"},
...
export default class Default extends Component {
  ...
  columns = [
    {
      ...
      template: (row) => {
        return (<React.Fragment>
                  <Button onClick={() => this.doDelete(row.id_tipe_tiang)} title="Hapus" className="btn-danger btn-brand icon btn-sm">
                    <i className="cui-trash"></i>
                  </Button> {'   '}
                  <Button onClick={() => this.doUpdate(row.id_tipe_tiang)} title="Ubah" className="btn-stack-overflow btn-brand icon btn-sm">
  ...
  getData(){
    ...
    axios.get(API().getMasterTiang + "?" + this.state.query, header(localStorage.getItem('token')))
    .then((response) => {
        let res = response.data;
        let result = SwalResult(res);
        if(result)
  ...
  doDelete(id){
    Swal.fire({
      ...
      if(result.value){
        SwalLoading();
        axios.post(API().deleteMasterTiang, params({id: id}), header(localStorage.getItem('token')))
        .then((response) => {
            let res = response.data;
            let result = SwalResult(res);
            if(result){
  ...
  doUpdate(id){
    ...
    axios.get(API().getMasterTiangBy + "?id="+id, header(localStorage.getItem('token')))
    .then((response) => {
        let res = response.data;
        let result = SwalResult(res);
        if(result)
