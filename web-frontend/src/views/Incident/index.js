...
import { SelectForm, DateForm, FileForm, InputForm } from '../../Component/FormComponent';
...
var moment = require('moment');
  ...
  function Add(props){
      ...
      const executeSubmit = (data, status) =>{
        ...
        axios.post(API().postIncident, formData, header(localStorage.getItem('token'), 'multipart/form-data'))
        .then((response) => {
                  ...
                  <Card>
                      ...
                      <CardBody>
                        <Form onSubmit={handleSubmit(onSubmit)}>
                          <Row>
                            ...
                            <Col md="6">
                              <DateForm 
                                  ...
                                  name   = "waktu_start"
                                  label  = "Waktu Incident"
                                  errors = {errors}
                                  onChange = {(value) => setMulai(value)}
                              />
                              <DateForm 
                                  refs={register({
                                      required : false,
                                  ...
                                  label  = "Waktu Selesai"
                                  errors = {errors}
                                  onChange = {(value) => setSelesai(value)}
                              />
                              <SelectForm 
                                  refs={register({
                                      required: true,
                          ...
                          :
                          <>
                          <Button type="button" onClick={handleSubmit(onDraft)} size="sm" color="primary" disabled={loading}> <span><i className="fa fa-dot-circle-o"></i> Simpan</span> </Button> 
                          {'    '}
                          <Button type="submit" size="sm" color="primary" disabled={cekReady() || loading}><span><i className="fa fa-dot-circle-o"></i> Selesai</span> </Button>
                          </>
                          }
                        </Form>
                      </CardBody>
  ...
  function Update(props){
    const { register, handleSubmit, errors, setValue } = useForm({
      defaultValues: {
        ...
        waktu_start  : props.data.waktu_start,
        waktu_finish : props.data.waktu_finish,
        keterangan   : props.data.keterangan,
        perbaikan   : props.data.perbaikan,
        id_status_inc: props.data.id_status_inc,
      }
    });
    const defaultValue                = [{value:'',name:'Loading...'}];
    const [loading, setLoading]       = useState(false);
    ...
    const [selesai, setSelesai]       = useState(props.data.waktu_finish);
    const [mulai, setMulai]           = useState(props.data.waktu_start);
    const [status, setStatus]         = useState(props.data.id_status_inc);
    ...
    const executeSubmit = (data, status) =>{
      ...
      formData.append('foto', JSON.stringify(foto));
      formData.append('status', status);
      axios.post(API().updateIncident, formData, header(localStorage.getItem('token'), 'multipart/form-data'))
      .then((response) => {
          let res = response.data;
          let result = SwalResult(res);
        ...
        <Row>
            <Col xs="12" sm="12" md="12">
                <Card>
                    ...
                    <CardBody>
                      <Form onSubmit={handleSubmit(onSubmit)}>
                        <input ref={register()} type="hidden" name="id" value={props.data.id_inc}/>
                        <Row>
                          <Col md="6">
                            <SelectForm 
                                refs={register({
                          ...
                          <Col md="6">
                            <DateForm 
                                ...
                                label  = "Waktu Incident"
                                errors = {errors}
                                value  = {props.data.waktu_start}
                                onChange = {(val) => setMulai(val)}
                            />
                            <DateForm 
                                refs={register({
                                    required: false,
                                ...
                                label  = "Waktu Selesai"
                                errors = {errors}
                                value  = {props.data.waktu_finish}
                                onChange = {(val) => setSelesai(val)}
                            />
                            <SelectForm 
                                refs={register({
                                    required: true,
    ...
    columns = [
      {
        ...
        template: (row) => {
          return (<React.Fragment>
                    <Button onClick={() => this.doDelete(row.id_inc)} title="Hapus" className="btn-danger btn-brand icon btn-sm">
                      <i className="cui-trash"></i>
                    </Button> {'   '}
                    <Button onClick={() => this.props.doUpdate(row.id_inc)} title="Ubah" className="btn-stack-overflow btn-brand icon btn-sm">
  ...
  class DraftList extends Component {
    ...
    getData(){
      this.setState({loading: true});
      axios.get(API().getIncident + "?status=1&" + this.state.query, header(localStorage.getItem('token')))
      .then((response) => {
          let res = response.data;
          let result = SwalResult(res);
          if(result){
  ...
  class CompleteList extends Component {
    ...
    getData(){
      this.setState({loading: true});
      axios.get(API().getIncident + "?status=2&" + this.state.query, header(localStorage.getItem('token')))
      .then((response) => {
          let res = response.data;
          let result = SwalResult(res);
          if(result)
  ...
  export default class Default extends Component {
      ...
      if(this.state.listactive){
        return (
          <>
            ...
            <TabPane tabId="2">
              <DraftList 
                total    = {(total) => this.setState({totaldraft: total})}
                doRead   = {(id) => this.doRead(id)}
                doUpdate = {(id) => this.doUpdate(id)}
              />
            </TabPane>
    ...
    getStatus(){
      axios.get(API().getOptionStatus, header(localStorage.getItem('token')))
      .then((response) => {
          let res = response.data;
          let result = SwalResult(res);
          if(result){
              this.setState({optionStatus: res.data});
          }
      }).catch((error) => {
          Swal.fire({
          title: 'Error!',
    ...
    doUpdate(id){
      ...
      axios.get(API().getIncidentBy + "?id="+id, header(localStorage.getItem('token')))
      .then((response) => {
          let res = response.data;
          let result = SwalResult(res);
          if(result)
          this.setState({dataUpdate: res.data.incident,fotoUpdate: res.data.foto, pageUpdate: true, updateLoading: false});
      }).catch((error) => {
        Swal.fire({
          title: 'Error!',
          text: error.toString(),
    ...
    render() {
        var viewAdd =  null;
        var viewTotal = null;
        var viewTotalDraft = null;
        if(this.state.optionCabang !== null && this.state.optionStatus !== null && this.state.optionKegiatan !== null && this.state.pageAdd){
            viewAdd = (<Add
                      ...
                      />)
        }else if(this.state.optionCabang !== null && this.state.optionStatus !== null && this.state.optionKegiatan !== null && this.state.pageUpdate){
            viewAdd = (<Update
        ...
        if(this.state.totaldraft !== null){
          viewTotalDraft = <>  {'  '}  <Badge pill color="danger">{this.state.totaldraft}</Badge> </>
        }
