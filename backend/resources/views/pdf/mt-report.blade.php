...
<body>
    <p align="center"><b>ACTIVITY REPORT <br/> A. MAINTENANCE REPORT {{$date}}</b></p>
    ...
    <table style="width: 100%;" class="table1">
        <thead>
            <tr>
                ...
                <th style = "width: 30%;">Maintenance</th>
            </tr>
        </thead>
        <tbody>
        <?php 
        ...
        @foreach($data as $key => $d)
            @foreach ($d['data'] as $k => $item)
                <tr>
                    ...
                    @if($k === 0)
                        @if($d['rows'] > 1)
                        <td rowspan="{{$d['rows']}}" align = "center">{{Sideveloper::getFullDate($item->waktu)}}</td>
                        @else
                        <td align = "center">{{Sideveloper::getFullDate($item->waktu)}}</td>
                        @endif
                    @endif
                    <td align = "center">{{Sideveloper::getHour($item->waktu)}}</td>
                    <td align = "center">{{$item->kode_cctv}}</td>
                    <td align = "center">{{$item->nama_cctv}}</td>
                    <td align = "center">{{$item->kegiatan}}</td>
                </tr>
            @endforeach
        @endforeach
        </tbody>
